...
'''选择一个下标不为i的随机整数，i是alpha的下标，m是alpha的数目'''
def selectJrand(i, m):
    j = i
    while j == i: # 返回一个不为i的随机数，在0~m之间的整数值
        j = int(random.uniform(0, m))
    return j


'''clipAlpha(调整aj的值，使aj处于 L<=aj<=H)，aj目标值'''
...
def plotfig_SVM(xMat, yMat, ws, b, alphas):
    ...
    # 注意flatten的用法,a.flatten()就是把a降到一维，默认是按横的方向降
